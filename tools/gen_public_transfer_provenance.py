...
def render_base_upstream_drift_section() -> list[str]:
    return [
        "## Upstream drift while PR #1293 waited (factual timeline)",
        "",
        "Original upstream PR `tenstorrent/pytorch2.0_ttnn#1293` was opened **2025-11-19** "
        "(head `254f2642`, 41 commits on `main`). Required checks passed "
        "(C++ Extension Tests, validate-pr). The PR was never reviewed or merged. While it "
        "remained open, `main` advanced through the same files this PR edits "
        "(`README.md`, `docs/README.md.in`, CI workflows), which produced the GitHub banner "
        "**\"This branch has conflicts that must be resolved\"** on an otherwise-green PR.",
        "",
        ...
        "| 2025-12-18 | Remove cron jobs from CI workflows | Joe Malone | "
        "`.github/workflows/run-accuracy-tests.yaml`, `update-readme.yaml`, `update-ttnn-wheel.yaml` | "
        "PR edits all three workflows |",
        ...
        "",
        "PR #1293 touches **10 files** vs `main` at open time: `README.md` (377 lines), "
        "`docs/README.md.in` (77 lines), and **8** workflow files under `.github/workflows/`.",
        "",
        "### Concrete delta absorbed by this rebase",
        "",
        "The rebased PR (`c81f7e56c`) already includes `#1310` and cron-removal in its "
        "ancestry and re-applies the build/CI/install changes on top. Example hunk from "
        "`#1310` now present in this branch:",
        "",
        "```diff",
        ...
        "```",
        "",
        "Cron schedule lines removed by maintainer merge (`ffd3b0e`) in the three workflows "
        "above are reconciled in this rebase. **No build, CI, or eager-op behavior change** "
        "was introduced solely to resolve these conflicts.",
        "",
        ...
        "| Shared commit subjects (51 in `#1293`) | **48/51** byte-identical by patch-id; "
        "3 differ only in rebase context (`build: modernize...`, Docker pin, CI checkbox) |",
...
def render_pr_body(entry: StackEntry) -> str:
    ...
    if entry.slug == "01-base":
        ...
        lines.extend(render_base_upstream_drift_section())
